      ...
      if (cmd.commandName === 'event') {
        ...
        if (sub === 'create') {
          ...
          const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder().setCustomId(`rsvp_${event.id}`).setLabel('✅ RSVP').setStyle(ButtonStyle.Success),
            new ButtonBuilder().setCustomId(`add_guests_${event.id}`).setLabel('➕ Guests').setStyle(ButtonStyle.Primary)
          );
          // Public post
        ...
        if (sub === 'list') {
          ...
          if (!Array.isArray(events) || events.length === 0) {
            ...
            const list = events.map((e: any) => `• **${e.name}** (ID: ${e.id}) — ${e.startDate}`).join('\n');
        ...
        const modal = new ModalBuilder()
          ...
          .addComponents(
            new ActionRowBuilder<ModalActionRowComponentBuilder>().addComponents(
              new TextInputBuilder().setCustomId('guestCount').setLabel('How many guests?').setStyle(TextInputStyle.Short).setRequired(true)
            ),
            new ActionRowBuilder<ModalActionRowComponentBuilder>().addComponents(
              new TextInputBuilder().setCustomId('guestNames').setLabel('Guest names (comma-separated or one per line)').setStyle(TextInputStyle.Paragraph)
            )
          );
