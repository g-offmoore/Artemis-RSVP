...
import { updateEventTypeAction, ActionState } from "../../actions";
...
export function SignupOptionsPanel({
  eventId,
  current,
}: {
  eventId: string;
  ...
  const [state, action, pending] = useActionState(updateEventTypeAction, emptyState);
  ...
  return (
    <section>
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1rem" }}>
        <h2 style={{ margin: 0 }}>Signup Options</h2>
        <button
      ...
      {expanded && (
        <form action={action} style={{ padding: "1rem", background: "var(--color-surface, #1a1a2e)", borderRadius: "4px" }}>
          ...
          <button type="submit" disabled={pending} className="button secondary">
            {pending ? "Saving…" : "Save signup options"}
          </button>
        </form>
      )}
    </section>
